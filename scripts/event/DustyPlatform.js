...
const BattlefieldFieldID = 910320100;
const RestingSpotFieldID = 910320010;
const ATTR_KILLS = "massacre_hit";
const ATTR_MISS = "massacre_miss";
const ATTR_COOL = "massacre_cool";

const EventName = "SubwayPQ";
let ID = 0
...
function timeout(eim) {
    ...
    eim.getPlayers().forEach(p => {
        p.changeMap(RestingSpotFieldID);
...
function playerEntry(eim, player) {
    if (eim.vars.destination == null) {
        player.dropMessage("The Dusty Platform is currently busy. Try again another time");
        return eim.dispose();
    }
...
function getUsableMap(fieldID) {
    ...
    if (!map.getCharacters().isEmpty()) {
        if (fieldID > BattlefieldFieldID + 4) return null;
        return getUsableMap(fieldID + 1);
    }
    map.killAllMonsters();
    map.clearDrops();
    return map.getId();
