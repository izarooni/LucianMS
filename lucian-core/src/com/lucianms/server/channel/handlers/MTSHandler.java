...
import com.lucianms.server.MTSItemInfo;
...
public final class MTSHandler extends PacketEvent {
        ...
        if (slea.available() > 0) {
            ...
            if (op == 2) { //put item up for sale
                            ...
                            try (ResultSet rs = ps.executeQuery()) {
                                if (rs.next()) {
                                    if (rs.getInt(1) > 10) { //They have more than 10 items up for sale already!
                                        ...
                                        c.announce(getMTS(con, 1, 0, 0));
                                        c.announce(MaplePacketCreator.transferInventory(getTransfer(con, c.getPlayer().getId())));
                                        c.announce(MaplePacketCreator.notYetSoldInv(getNotYetSold(con, c.getPlayer().getId())));
                                        return;
                                    }
                                }
                            }
                ...
                try (Connection con = c.getChannelServer().getConnection()) {
                    ...
                    if (tab == 4 && type == 0) {
                        c.announce(getCart(con, c.getPlayer().getId()));
                    } else if (tab == c.getPlayer().getCurrentTab() && type == c.getPlayer().getCurrentType() && c.getPlayer().getSearch() != null) {
                        c.announce(getMTSSearch(con, tab, type, c.getPlayer().getCurrentCI(), c.getPlayer().getSearch(), page));
                    } else {
                        c.getPlayer().setSearch(null);
                        c.announce(getMTS(con, tab, type, page));
                    }
                ...
                try (Connection con = c.getChannelServer().getConnection()) {
                    c.announce(getMTSSearch(con, tab, type, ci, search, c.getPlayer().getCurrentPage()));
                ...
                try (Connection con = c.getChannelServer().getConnection()) {
                    ...
                    c.announce(MaplePacketCreator.enableCSUse());
                    c.announce(getMTS(con, c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                    c.announce(MaplePacketCreator.notYetSoldInv(getNotYetSold(con, c.getPlayer().getId())));
                    c.announce(MaplePacketCreator.transferInventory(getTransfer(con, c.getPlayer().getId())));
                } catch (SQLException e) {
                    e.printStackTrace();
    ...
    public List<MTSItemInfo> getNotYetSold(Connection con, int cid) throws SQLException {
    ...
    public byte[] getCart(Connection con, int cid) throws SQLException {
        List<MTSItemInfo> items = new ArrayList<>();
        int pages = 0;
    ...
    public List<MTSItemInfo> getTransfer(Connection con, int cid) throws SQLException {
        List<MTSItemInfo> items = new ArrayList<>();
    ...
    private static byte[] getMTS(Connection con, int tab, int type, int page) throws SQLException {
        List<MTSItemInfo> items = new ArrayList<>();
        int pages = 0;
        String query;
        if (type != 0) {
            query = "SELECT * FROM mts_items WHERE tab = ? AND type = ? AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
        } else {
            query = "SELECT * FROM mts_items WHERE tab = ? AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
        }
    ...
    public byte[] getMTSSearch(Connection con, int tab, int type, int cOi, String search, int page) throws SQLException {
        List<MTSItemInfo> items = new ArrayList<>();
        ...
        String query;
        if (type != 0) {
            query = "SELECT * FROM mts_items WHERE tab = ? " + listaitems + " AND type = ? AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
        } else {
            query = "SELECT * FROM mts_items WHERE tab = ? " + listaitems + " AND transfer = 0 ORDER BY id DESC LIMIT ?, 16";
        }
        try (PreparedStatement ps = con.prepareStatement(query)) {
            ps.setInt(1, tab);
            if (type != 0) {
