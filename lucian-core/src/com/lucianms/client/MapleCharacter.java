...
public class MapleCharacter extends AbstractAnimatedMapleMapObject implements Disposable {
    ...
    private int easyBossPoints;
    private int normalBossPoints;
    private int hardBossPoints;
    private int hellBossPoints;
    //endregion
    ...
    private EnumMap<MapleBuffStat, BuffContainer> effects = new EnumMap<>(MapleBuffStat.class);
        ...
        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM characters WHERE id = ?")) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                if (channelserver) {
                    ...
                    ret.easyBossPoints = rs.getInt("easy_bosspoints");
                    ret.normalBossPoints = rs.getInt("normal_bosspoints");
                    ret.hardBossPoints = rs.getInt("hard_bosspoints");
                    ret.hellBossPoints = rs.getInt("hell_bosspoints");
                    ...
                    if (oOrdinal > -1) {
                        ...
                        ret.occupation.setExperience(rs.getInt("occupation_exp"));
                    }
...
/*            else {
    ...
    public void cancelAllBuffs() {
        cancelBuffs(new HashSet<>(effects.keySet()));
    }

    public void cancelBuffStats(MapleBuffStat stat) {
        Set<MapleBuffStat> set = Set.of(stat);
        deregisterBuffStats(set);
        cancelPlayerBuffs(set);
    }
    ...
    public void setCombo(short count) {
        ...
        setLastCombo(System.currentTimeMillis());
        */
        combocounter = (short) Math.min(30000, count);
    ...
    public void clearBuffs() {
        for (BuffContainer container : effects.values()) {
            container.dispose();
        }
        effects.clear();
    ...
    public EnumMap<MapleBuffStat, BuffContainer> getEffects() {
        return effects;
    }
    ...
    public void cancelBuffs(Set<MapleBuffStat> buffs) {
        ...
        updateLocalizedStats();
        enforceMaxHpMp();
        announce(MaplePacketCreator.getResetTempStats(buffs));
        if (!buffs.isEmpty()) {
            getMap().sendPacketExclude(MaplePacketCreator.getResetRemoteTempStats(getId(), buffs), this);
        }
    }

    public void cancelEffect(int itemId) {
        cancelEffect(MapleItemInformationProvider.getInstance().getItemEffect(itemId), -1, false);
    }

    public void cancelEffect(MapleStatEffect effect, long startTime, boolean overwrite) {
        Set<MapleBuffStat> toRemove;
        if (!overwrite) {
            toRemove = getBuffStats(effect, startTime);
        } else {
            Map<MapleBuffStat, Integer> stats = effect.getStatups();
            toRemove = new HashSet<>(stats.size());
            toRemove.addAll(stats.keySet());
        }
        deregisterBuffStats(toRemove);
    ...
    public void changeJob(MapleJob newJob) {
        ...
        if (GameConstants.hasSPTable(newJob) && newJob.getId() != 2001) {
            if (getBuffedValue(MapleBuffStat.RIDE_VEHICLE) != null) {
                cancelBuffStats(MapleBuffStat.RIDE_VEHICLE);
            }
            createDragon();
        }
    }
    ...
    private void deregisterBuffStats(final Set<MapleBuffStat> stats) {
        List<BuffContainer> effectsToCancel = new ArrayList<>(stats.size());
        for (MapleBuffStat stat : stats) {
            BuffContainer mbsvh = effects.get(stat);
            if (mbsvh != null) {
                effects.remove(stat);
                boolean addMbsvh = true;
                for (BuffContainer contained : effectsToCancel) {
                    if (mbsvh.getStartTime() == contained.getStartTime() && contained.getEffect().isSameSource(mbsvh.getEffect())) {
                        addMbsvh = false;
                    }
                }
                if (addMbsvh) {
        ...
        for (BuffContainer container : effectsToCancel) {
            TaskExecutor.cancelTask(container.getTask());
            cancelEffect(container.getEffect(), -1, false);
        }
    }
    ...
    public void dispel() {
        for (BuffContainer container : new ArrayList<>(effects.values())) {
            if (container.getEffect().isSkill()) {
                cancelEffect(container.getEffect(), container.getStartTime(), false);
            }
        }
    }
    ...
    public void giveDebuff(final MapleDisease disease, MobSkill skill) {
        ...
        if (!hasDisease(disease) && diseases.size() < 2) {
            if (!(disease == MapleDisease.SEDUCE || disease == MapleDisease.STUN)) {
                if (effects.containsKey(MapleBuffStat.HOLY_SHIELD)) {
                    return;
                }
            }
    ...
    public void dispelSkill(int skillID) {
        ...
        if (skill != null && skillLevel > 0) {
            ...
            if (effect != null) {
                ...
                Set<MapleBuffStat> remove = getEffects().entrySet().stream()
    ...
    public void gainMeso(int gain, boolean show, boolean enableActions, boolean inChat) {
        ...
        if (isAutoCurrency() && meso.get() >= 2000000000) {
            if (MapleInventoryManipulator.checkSpace(getClient(), ServerConstants.CURRENCY, 1, "")) {
                ...
                if (occupation != null && occupation.getType() == Occupation.Type.Miser) {
                    if (occupation.gainExperience(80)) {
    ...
    public ArrayList<BuffContainer> getAllBuffs() {
    ...
    public Integer getBuffedValue(MapleBuffStat effect) {
        BuffContainer container = effects.get(effect);
        return container == null ? null : container.getValue();
    }

    private Set<MapleBuffStat> getBuffStats(MapleStatEffect effect, long startTime) {
        Set<MapleBuffStat> stats = new HashSet<>();
        for (Entry<MapleBuffStat, BuffContainer> e : effects.entrySet()) {
            if (e.getValue().getEffect().isSameSource(effect) && (startTime == -1 || startTime == e.getValue().getStartTime())) {
                stats.add(e.getKey());
            }
        }
        return stats;
    ...
    public void setLevel(int level) {
        this.level = Math.min(255, Math.max(0, level));
    }
    ...
    public boolean isActiveBuffedValue(int skillid) {
        return effects.values().stream().filter(e -> e.getEffect() != null
                && e.getEffect().isSkill()
                && e.getEffect().getSourceId() == skillid).findAny().isPresent();
    }
    ...
    public boolean isBuffFrom(MapleBuffStat stat, Skill skill) {
        BuffContainer container = effects.get(stat);
        if (container == null) {
            return false;
        }
        return container.getEffect().isSkill() && container.getEffect().getSourceId() == skill.getId();
    }
    ...
    public void gainLevels(int levels) {
        ...
        Occupation occupation = getOccupation();
        if (occupation != null) {
            if (occupation.getType() == Occupation.Type.Hoarder) {
                MaplePet pet = getPet(0);
                if (pet != null) {
                    MapleMap.doItemVac(this, pet, 100 * (occupation.getLevel() + 1));
                    if (occupation.gainExperience(8)) {
                        sendMessage("Your occupation is now level {}", occupation.getLevel());
                    }
                }
            }
...
//            getMap().broadcastMessage(this, MCarnivalPacket.getMonsterCarnivalPlayerDeath(this));
        ...
        if (getBuffedValue(MapleBuffStat.MORPH) != null) {
            cancelBuffs(Set.of(MapleBuffStat.MORPH));
        }
    ...
    public void updateLocalizedStats() {
        ...
        magic = Math.min(magic, 2147483647);
        Integer hbhp = getBuffedValue(MapleBuffStat.MAX_HP);
        if (hbhp != null) {
            localmaxhp += (hbhp.doubleValue() / 100) * localmaxhp;
        }
        Integer hbmp = getBuffedValue(MapleBuffStat.MAX_MP);
        if (hbmp != null) {
        ...
        BuffContainer combo = getEffects().get(MapleBuffStat.COMBO_COUNTER);
        if (combo != null) {
            watk += combo.getValue();
        }
        ...
        Integer mwarr = getBuffedValue(MapleBuffStat.BASIC_STAT_UP);
        if (mwarr != null) {
    ...
    public void saveToDB() {
        ...
        try (Connection con = client.getWorldServer().getConnection()) {
            ...
            try (PreparedStatement ps = con.prepareStatement("UPDATE characters\n" +
                ...
                ps.setInt(61, easyBossPoints);
                ps.setInt(62, normalBossPoints);
...
/*
        ...
        if (occupation != null) {
            ...
            switch (occupation.getType()) {
                case Grinder:
                    expRate += (int) Math.floor(expRate * ((double)(occupationLevel) / 24));
                    mesoRate -= (int) Math.floor(mesoRate * ((double)(occupationLevel) / 75));
                    dropRate -= 1;
                    break;
                case Miser:
                    mesoRate += (int) Math.floor(mesoRate * ((double)(occupationLevel)));
                    expRate -= (int) Math.floor(expRate * ((double)(occupationLevel) / 75));
                    dropRate -= 1;
                    break;
                case Hoarder:
                    dropRate += 1 + (int) Math.floor((double)(occupationLevel) / 3);
                    mesoRate -= (int) Math.floor(mesoRate * ((double)(occupationLevel) / 75));
                    expRate -= (int) Math.floor(expRate * ((double)(occupationLevel) / 75));
                    break;
                case FM_WHORE:
...
*/
    ...
    public void silentGiveBuffs(Collection<BuffContainer> buffs) {
        buffs.stream().filter(b -> b.getEffect() != null).forEach(b -> b.getEffect().applyTo(this));
    }
    ...
    public int getEasyBossPoints() {
        return easyBossPoints;
    }

    public void setEasyBossPoints(int easyBossPoints) {
        this.easyBossPoints = easyBossPoints;
    }

    public int getNormalBossPoints() {
        return normalBossPoints;
    }

    public void setNormalBossPoints(int normalBossPoints) {
        this.normalBossPoints = normalBossPoints;
    }

    public int getHardBossPoints() {
        return hardBossPoints;
    }

    public void setHardBossPoints(int hardBossPoints) {
        this.hardBossPoints = hardBossPoints;
    }

    public int getHellBossPoints() {
        return hellBossPoints;
    }

    public void setHellBossPoints(int hellBossPoints) {
        this.hellBossPoints = hellBossPoints;
    }
    ...
    public void doRebirth() {
        ...
        if (getOccupation() != null) {
            if (getOccupation().getType() == Occupation.Type.Grinder) {
                rebirthPoints += (int) Math.floor(getOccupation().getLevel() / 3);
                occupation.gainExperience(35);
            }
        }
        /*if (!isAutoRebirth()) {
            job = MapleJob.BEGINNER;
    ...
    public boolean addPoints(String pointType, int amount) {
        switch (pointType) {
            ...
            case "ep": // event points
                Occupation occupation = getOccupation();
                if (occupation != null && occupation.getType() == Occupation.Type.FM_WHORE) {
                    setEventPoints(getEventPoints() + (2 * amount));
                } else {
            ...
            case "vp": // vote points
                ...
                getClient().setVotePoints(getClient().getVotePoints() + amount);
                return true;
            case "nx": // nx cash
            ...
            case "rp":
                ...
                return true;
            case "easy":
                setEasyBossPoints(getEasyBossPoints() + amount);
                return true;
            case "normal":
                setNormalBossPoints(getNormalBossPoints() + amount);
                return true;
            case "hard":
                setHardBossPoints(getHardBossPoints() + amount);
                return true;
            case "hell":
                setHellBossPoints(getHellBossPoints() + amount);
                return true;
