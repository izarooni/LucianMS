...
public class MapleClient implements Disposable {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts WHERE name = ?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                       ...
                       if(!Collections.disjoint(this.getHardwareIDs(), BanManager.getHWIDs())){
                            banReason = "You've been banned by association";
                            return 3;
                        }
    ...
    public String getMAC(){
        ...
        String mac="";
        try (Connection con = Server.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("select mac from accounts_macs where account_id = ?")) {
                ps.setInt(1, getAccID());
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
    ...
    public void dispose() {
        ...
        session.close(); // closing the session invokes the MapleClient#disconnect method
        ID = 0;
        player = null;
    ...
    public int getDonor() {
        try (Connection con = Server.getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT donor FROM accounts WHERE id = ?")) {
            ps.setInt(1, ID);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
            ...
            LOGGER.warn("Unable to retrieve donor status for account {} player {}", getAccountName(), ((player != null) ? player.getName() : "N/A"), e);
        }
        return 0;
    }

    public void setDonor(int n) {
        try (Connection con = Server.getConnection();
             PreparedStatement ps = con.prepareStatement("UPDATE accounts SET donor = ? WHERE id = ?")) {
            ps.setInt(1, n);
            ps.setInt(2, ID);
            ps.executeUpdate();
        } catch (SQLException e) {
            LOGGER.warn("Unable to update donor status for account {} player {}", getAccountName(), ((player != null) ? player.getName() : "N/A"), e);
        }
    }
