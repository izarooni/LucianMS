...
public class GMCommands implements CommandPattern {
...

    {
        ...
        commands.add("!clearslot <inventory> - clear all items in a specific inventory type");
    }
        ...
        if (command.startsWith("!")) {
            ...
            if (command.equalsIgnoreCase("help") || command.equalsIgnoreCase("commands")) {
                ...
                if (args.length == 2) {
                    ...
                    for (int i = (commands.size() > 10 ? Integer.parseInt(args[1]) * 10 : 0); i < (commands.size() > 10 ? Integer.parseInt(args[1]) * 10 + 10 : commands.size()); i++) {
                        if (!(i > commands.size())) {
                ...
                if (args.length == 2) {
                    if (args[1].equalsIgnoreCase("map")) {
                        ...
                        MapleCharacter target = Server.getInstance().getWorld(player.getWorld()).getPlayerStorage().getCharacterByName(args[1]);
                ...
                if (args.length >= 2) {
                    ...
                    MapleMap map = Server.getInstance().getWorld(player.getWorld()).getChannel(player.getClient().getChannel()).getMapFactory().getMap(id);
                        ...
                        case "right":
                            if (args.length >= 3) {
                                if (args[2].equalsIgnoreCase("map")) {
                                    ...
                                    MapleCharacter toMove = player.getClient().getChannelServer().getPlayerStorage().getCharacterByName(args[2]);
                                    if (toMove != null) {
                        ...
                        case "winner":
                            if (Events.getInstance().isActive()) {
                                switch (args[2].toLowerCase()) {
                                    case "add":
                                        if (args.length >= 4) {
                                            MapleCharacter target = player.getClient().getChannelServer().getPlayerStorage().getCharacterByName(args[3]);
                                            if (target != null) {
                                                if (Events.getInstance().getParticipants().containsKey(target.getId())) {
                                                    int amount = 1;
                                                    if (args.length >= 5) {
                                    ...
                                    case "remove":
                                        if (args.length >= 4) {
                                            MapleCharacter target = Server.getInstance().getWorld(player.getWorld()).getPlayerStorage().getCharacterByName(args[3]);
                                            if (target != null) {
                                                int amount = 1;
                                                if (args.length >= 5) {
                                                    try {
                ...
                if (args.length >= 2) {
                    ...
                    for (byte i = 0; i < inventory.getSlotLimit(); i++) {
                        ...
                        if ((item = inventory.getItem(i)) != null) {
                            ...
                            MapleInventoryManipulator.removeById(c, iType, itemId, quantity, false, false);
                        }
                    }
                } else {
