...
public final class EnterMTSHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        try (Connection con = c.getChannelServer().getConnection()) {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM mts_items")) {
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        pages = (int) Math.ceil(rs.getInt(1) / 16f);
                    }
                }
            }
            c.announce(MaplePacketCreator.sendMTS(items, 1, 0, 0, pages));
            c.announce(MaplePacketCreator.transferInventory(getTransfer(con, chr.getId())));
            c.announce(MaplePacketCreator.notYetSoldInv(getNotYetSold(con, chr.getId())));
        } catch (SQLException e) {
            logger().error("Unable to create database connection: {}", e.getMessage());
        }
    }

    private List<MTSItemInfo> getNotYetSold(Connection con, int cid) {
        List<MTSItemInfo> items = new ArrayList<>();
        try {
    ...
    private List<MTSItemInfo> getTransfer(Connection con, int cid) {
        List<MTSItemInfo> items = new ArrayList<>();
