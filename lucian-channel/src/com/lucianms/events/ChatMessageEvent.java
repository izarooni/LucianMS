...
public class ChatMessageEvent extends PacketEvent {
    ...
    public Object onPacket() {
        ...
        if (!player.isMuted() || player.isGM()) {
            ...
            Object ntm = player.getMap().getVariables().get("ntm");
            if (ntm != null) {

                @SuppressWarnings("unchecked") Pair<String, MapleMonster> pair = (Pair<String, MapleMonster>) ntm;
                if (content.equalsIgnoreCase(pair.getLeft())) {
                    pair.getRight().disableDrops();
                    player.getMap().killAllMonsters();
                    // removed on next field  tick
                    player.getMap().getVariables().remove("ntm");
                    onPost(() -> player.getMap().sendMessage(2, "NTM : {} has guessed the name of the monster correctly!", player.getName()));
                }
            }
            if (!player.isHidden()) {
