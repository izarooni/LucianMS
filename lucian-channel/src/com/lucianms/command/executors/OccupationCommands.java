...
public class OccupationCommands {
    ...
    public static boolean execute(MapleClient client, Command command, CommandArgs args) {
        ...
        if (command.equals("occupation")) {
            ...
            switch (type) {
                default:
                    player.sendMessage("Unfortunately, your talent ('{}') does not come with any commands.", type.name());
                    break;
                case FM_WHORE:
            ...
            return true;
                ...
                && ToggleAutoCoin(client, command, args)) {
            return true;
        }
        return false;
    }

    private static boolean ToggleAutoCoin(MapleClient client, Command command, CommandArgs args) {
        MapleCharacter player = client.getPlayer();
