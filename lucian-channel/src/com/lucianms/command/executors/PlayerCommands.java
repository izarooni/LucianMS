...
public class PlayerCommands extends CommandExecutor {
    ...
    public PlayerCommands() {
        ...
        addCommand("save", this::Save, "Saves your character");

        addCommand("go", this::Go, "Fast warp to towns and features");
        addCommand("warper", this::Warp, "Opens Warp NPC");
...
//        addCommand("afk", this::SetAFK);
        ...
        addCommand("cleardrops", this::ClearDrops, "Clears drops in your map");
    ...
    private void Save(MapleCharacter player, Command cmd, CommandArgs args){
        player.saveToDB();
    ...
    private void ClearDrops(MapleCharacter player, Command cmd, CommandArgs args){
        if(player.getMapId() != 109020001 && player.getMapId() != ServerConstants.HOME_MAP) {
    ...
    private void DonorChat(MapleCharacter player, Command cmd, CommandArgs args){
        if(player.getClient().getDonor()>= 1){
            if (args.length() == 1) {
    ...
    private void Warp(MapleCharacter player, Command cmd, CommandArgs args) {
        if (!JailManager.isJailed(player.getId())) {
            NPCScriptManager.start(player.getClient(), 9899974, "9899974");
        } else {
            player.sendMessage(5, "You cannot do that while jailed.");
        }
    }
    ...
    private void BossHP(MapleCharacter player, Command cmd, CommandArgs args) {
        if(player.getMapId() == 109020001) {
            player.sendMessage(5, "You cannot use @bosshp here. Stop trying to cheat!");
        }
        else {
    ...
    private void CheckPlayer(MapleCharacter player, Command cmd, CommandArgs args) {
        ...
        player.sendMessage("NX: {}", StringUtil.formatNumber(target.getCashShop().getCash(1)));
    ...
    private void MaxStat(MapleCharacter player, Command cmd, CommandArgs args){
        if (args.length() == 0) {

            int required = 32763*4;
            int total = 0;
            ...
            if(required > player.getRemainingAp()){
                player.dropMessage("Make sure you have enough AP to distribute");
            }
            else{
                player.setStr(32767);
                ...
                player.setRemainingAp(player.getRemainingAp() - required);
                player.updateSingleStat(MapleStat.AVAILABLEAP, player.getRemainingAp());
            }
    ...
    private void Report(MapleCharacter player, Command cmd, CommandArgs args) {
        if (args.length() > 1) {
            ...
            if (target != null) {
                String content = String.format("%s: %s %s", player.getName(), username, message);
                world.sendMessageIf(p -> p.getGMLevel() > 0, 6, "[Report] {}", content);
            } else {
