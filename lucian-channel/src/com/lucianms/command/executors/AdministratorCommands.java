...
//        addCommand("forceevent", this::CommandForceEvent, "Force proc of an auto-event");
        ...
        addCommand("msi", this::CreateMSI, "Creates a MSI");
        addCommand("setdonor", this::CommandSetDonorLevel, "Sets donor status");
    ...
    private void CreateMSI(MapleCharacter player, Command cmd, CommandArgs args){
        if(args.length() != 2){
            player.sendMessage("usage: !{} <itemid> <tier>", cmd.getName());
            return;
        }

        Equip modified = MapleItemInformationProvider.getInstance().getEquipById(args.parseNumber(0, int.class));

        if(modified == null){
            player.sendMessage("You didn't provide an Equip ID");
            return;
        } else {
            int tier = args.parseNumber(1, int.class);
            short stat = 1;
            switch(tier){
                case 1:
                    stat = ServerConstants.TIER1;
                    break;
                case 2:
                    stat = ServerConstants.TIER2;
                    break;
                case 3:
                    stat = ServerConstants.TIER3;
                    break;
                case 4:
                    stat = ServerConstants.TIER4;
                    break;
                default:
                    player.sendMessage("Tiers should be a number from 1 - 4");
            }
            ...
            MapleInventoryManipulator.addFromDrop(player.getClient(), modified, true);
        }
    ...
    private void CommandSetDonorLevel(MapleCharacter player, Command cmd, CommandArgs args) {
        if (args.length() == 1) {

            MapleWorld world = player.getClient().getWorldServer();
            MapleCharacter target = world.getPlayerStorage().find(p -> p.getName().equalsIgnoreCase(args.get(0)));
            if (target != null) {
                target.getClient().setDonor(1);
                target.sendMessage(6, "They are now a donor");
                player.sendMessage(6, "Success!");
            } else {
                player.sendMessage(5, "Unable to find any player named '{}'", args.get(0));
            }
        } else {
            player.sendMessage(5, "usage: !setDonor <username>");
        }
    }
